...
<head>
    ...
    <title>Edit Pengguna - Sistem Monitoring Program Kerja Unit KEK BP Batam</title>
            ...
            <div class="flex-1 bg-black p-8 overflow-y-auto">
                ...
                <div class="mb-8">
                    <div class="flex items-center justify-between mb-6">
                        <h1 class="text-white text-4xl font-semibold">Edit Pengguna</h1>
                        <div class="flex gap-3">
                            <a href="{{ route('pengguna.show', $user) }}" class="bg-neutral-800 hover:bg-neutral-700 text-gray-300 px-4 py-2.5 rounded-lg text-sm">
                                👁️ Lihat Detail
                            </a>
                ...
                <div class="bg-neutral-900 rounded-2xl p-8">
                    <form method="POST" action="{{ route('pengguna.update', $user) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            <label for="role" class="block text-white text-sm font-medium mb-2">
                                Role Pengguna <span class="text-red-400">*</span>
                            </label>
                            <select name="role" id="role"
                                    class="w-full bg-neutral-800 text-gray-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 border border-neutral-700">
                                <option value="staf" {{ old('role', $user->role) === 'staf' ? 'selected' : '' }}>Staf</option>
                                <option value="kabag" {{ old('role', $user->role) === 'kabag' ? 'selected' : '' }}>Kepala Bagian</option>
                            </select>
                            @error('role')
                                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="flex gap-4 pt-4">
                            ...
                            <a href="{{ route('pengguna.index') }}" class="bg-neutral-800 hover:bg-neutral-700 text-gray-300 font-medium py-3 px-6 rounded-lg text-sm transition">
                                Batal
                            </a>
                        </div>
                    </form>
