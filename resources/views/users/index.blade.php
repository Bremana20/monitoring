...
<head>
    ...
    <title>Kelola Pengguna - Sistem Monitoring Program Kerja Unit KEK BP Batam</title>
            ...
            <div class="flex-1 bg-black p-8 overflow-y-auto">
                ...
                <div class="mb-8">
                    <div class="flex items-center justify-between mb-6">
                        <h1 class="text-white text-4xl font-semibold">Kelola Pengguna</h1>
                        <div class="flex gap-3">
                            <a href="{{ route('pengguna.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg text-sm flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-neutral-900 rounded-2xl p-6 border border-neutral-800 hover:border-blue-500 transition">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex-1">
                                <div class="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center text-white font-bold text-lg mb-3">
                                    {{ strtoupper(substr($user->name, 0, 1)) }}
                                </div>
                                <h4 class="text-white font-medium mb-1">{{ $user->name }}</h4>
                        ...
                        <div class="text-xs text-gray-500 mb-4">
                            ...
                            <p>Terakhir aktif: {{ $user->updated_at->diffForHumans() }}</p>
                        </div>

                        <div class="flex gap-2">
                            <a href="{{ route('pengguna.show', $user) }}" class="flex-1 bg-neutral-800 hover:bg-neutral-700 text-gray-300 px-3 py-2 rounded-lg text-sm text-center transition">
                                👁️ Lihat
                            </a>
                            <a href="{{ route('pengguna.edit', $user) }}" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg text-sm text-center transition">
                                ✏️ Edit
                            </a>
                            @if($user->id !== auth()->id())
                            <form method="POST" action="{{ route('pengguna.destroy', $user) }}" class="flex-1" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengguna ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded-lg text-sm transition">
                                    🗑️ Hapus
                                </button>
                            </form>
                            @endif
                    ...
                    <div class="col-span-full text-center py-12">
                        ...
                        <h3 class="text-white text-lg font-medium mb-2">Belum ada pengguna</h3>
                        <p class="text-gray-400 mb-4">Mulai dengan menambahkan pengguna pertama</p>
                        <a href="{{ route('pengguna.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg inline-flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/>
                            </svg>
                            Tambah Pengguna
